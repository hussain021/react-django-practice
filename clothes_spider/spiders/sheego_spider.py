...
class SheegoSpider(scrapy.Spider):
    ...
    def parse_product(self, response):
        ...
        """
        yield ProductItem(
            title = self._clean([self.get_title(response)])[0],
            price = eval([size.replace(',','.') for size in [self.get_price(response)[:-2]]][0]), 
            currency = self._clean([self.get_price(response)[-2:]])[0],
            image_urls = self.format_image_urls(self.get_image_urls(response)),
            all_sizes = [eval(size) for size in self._clean(self.get_all_sizes(response))],
            all_colors = self._clean(self.get_all_colors(response)),
            description = self._clean([self.get_description(response)])[0],
        )
